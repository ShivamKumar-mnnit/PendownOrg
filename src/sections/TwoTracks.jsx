  ...
  return (
    <section className="border-y border-(--color-border) bg-(--color-surface-alt)">
      <div className="mx-auto max-w-6xl px-5 py-20">
        <SectionHeading eyebrow="Pick Your Track" title="Two ways to prep with PenDown" />

        <div className="mt-12 grid gap-6 sm:grid-cols-2">
          {TRACKS.map(({ icon: Icon, title, tagline, points }) => (
            <div key={title} className="rounded-2xl border border-(--color-border) bg-(--color-surface) p-8">
              ...
              <Link
                to="/book"
                className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-(--color-accent) hover:opacity-75 transition-opacity"
              >
                Book {title}
